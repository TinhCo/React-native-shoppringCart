...
const getCoupons = asyncHandler(async (req, res) => {
    ...
    return res.json({
        success: response ? true : false,
        coupons: response ? response : 'Cannot create new Coupon'
    });
});

const updateCoupon = asyncHandler(async (req, res) => {
    const {cid} =  req.params
    if (Object.keys(req.body).lenth === 0) throw new Error('Missing inputs')
...
const deleteCoupon = asyncHandler(async (req, res) => {
    const {cid} =  req.params
    const response = await Coupon.findOneAndDelete(cid)
    return res.json({
        success: response ? true : false,
